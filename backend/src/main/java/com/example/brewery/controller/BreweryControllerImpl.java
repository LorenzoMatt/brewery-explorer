...
public class BreweryControllerImpl implements BreweryController {
    ...
    @Autowired
    public BreweryControllerImpl(BreweryService breweryService) {
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "500", description = "Internal Server Error", content = @Content(mediaType = "application/json", schema = @Schema(implementation = ErrorResponse.class))) })
    @GetMapping("/search")
    public ResponseEntity<List<BreweryDto>> searchBreweries(@RequestParam(required = false) String name,
